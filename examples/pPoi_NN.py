...
# Regularization parameter in the normalization layer
reg = 1e-8
...
for k in range(2,My):
    for j in range(2,Mx):
        ...
        data = io.loadmat("data_pPoi/data_Mx{:d}_My{:d}_({:d},{:d})_Ntrain{:d}_dxb{:.3e}.mat".format(Mx, My, j, k, Nt, Dx_buffer))
        data_init = io.loadmat("data_pPoi/init_Mx{:d}_My{:d}_({:d},{:d}).mat".format(Mx, My, j, k))
        ...
        class Net(nn.Module):
            ...
            def forward(self, x):
                # Normalized input
                Ex = torch.mean(x,1,keepdim=True)
                x = x - Ex
                Varx = np.sqrt(dx) * LA.norm(x,ord=2,dim=1,keepdim=True)
                Varx = torch.clamp(Varx,min=reg)
                x = x / Varx
